...
puzzles = [main.Puzzle("070583020059200300340006507795000632003697100680002700914835076030701495567429013"),
           ...
           main.Puzzle(
               "",
               4),
           main.Puzzle(
               "",
               4),
           ]
...
def test_completed():
    assert puzzles[0].completed is False  # Not complete
    assert puzzles[7].completed is True  # Complete


def test_load_game():
    assert game.puzzle is None  # Before loading puzzle
    assert (game.load_game("user",
                           "070583020059200300340006507795000632003697100680002700914835076030701495567429013")
            is True)  # Valid user input
    assert (game.load_game("user",
                           "07058302005920030034000650779500063200369710068000270091483507603070149556742901")
            is False)  # Invalid user input
    assert ("".join([cell.val for cell in game.puzzle.squares]) ==
            "070583020059200300340006507795000632003697100680002700914835076030701495567429013")
    assert game.load_game("load", "10") is True  # Valid index
    assert game.load_game("load", "1000") is False  # Invalid index
    assert game.load_game("load_random", "hard") is True  # Random puzzle
    assert [square.val for square in game.puzzle.squares].count("0") >= 42
    assert game.load_game("generate", "20") is True  # Generate sudoku
...
def test_add_guess():
    puzzles[0].add_guess(1, "2")  # Add guess
    puzzles[0].add_guess(1, "1")  # All smaller guess to cell
    assert puzzles[0].squares[1].guesses == ["1", "2"]
    puzzles[0].add_guess(1, "0")  # Clear guesses
    assert puzzles[0].squares[1].guesses == []


def test_add_all_guesses():
    puzzles[0].add_all_guesses()
    assert puzzles[0].squares[0].guesses == ["1"]  # Single valid value
    assert puzzles[0].squares[1].guesses == []  # Original cell
    assert puzzles[0].squares[2].guesses == ["1", "6"]  # Multiple valid values


def test_return_clashes():
    assert puzzles[1].return_clashes() == set()  # No clashes
    assert puzzles[4].return_clashes() == set([0, 7])  # Clashses present
    assert puzzles[7].return_clashes() == set()  # Complete
